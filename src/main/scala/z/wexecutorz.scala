...
object Wexecutors:
        extension [R, E, A](self: ZIO[R, E, A])
                /** Forks `that` in the background and immediately returns the result of `self`. The fiber launched by `that` is never joined, so it continues running in the
                  * background in Global Scope
                  */
