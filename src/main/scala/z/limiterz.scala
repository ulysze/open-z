...
/** A concurrency control mechanism that limits the rate at which operations can be performed. For handling errors and using the
  * Full Power of Scala. Use Pattern Matching to deconstruct the Exception and find the Context then compare with Types and value!
  * This is super powerful for handling toMuchRequest messages orient in locations manage logic etc. We recommand to create
  * registries of RateLimiter and then use PAttern mathcing to decompose. Create différente categories of rateLimiters etc. This
  * File provide a super powerful way to organize your Rate Limiting Logic. The user can create is Own Contexts... -> We give
  * Examples/ Possible Implemenations. To implement a RateLimiter, the User Must Define a context... -> Le laisser abstrait
  * alors.. Pattern Commun -> Peut-être utilisé trait ? -> A voir. TODO: Make it generic with the Context C -> Allows users to
  *
  * @param rateLimit
